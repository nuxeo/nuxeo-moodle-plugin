...
<link rel="stylesheet" media="screen" type="text/css" title="Designtab"
	href="designtab.css" />
</head>
<body>
	Execute a "SELECT * FROM Document WHERE ecm:path = Path" query to Nuxeo
	<br /> and print all the document properties.
	<br /> fill the path field with a correct Path and the Schema field
	<br /> with the type of schema to output (if left blank, print all
	properties)
	<br />

	<form action="B1.php" method="post">
		Path<input type="text" name="path" /> Schema<input type="text"
			name="schema" /> <input type="submit" value="Submit" />
	</form>
	<br />
<?php
...
function openDocumentPropeties($path, $propertiesSchema = '*') {
	...
	$interceptor = new NuxeoRequestInterceptor('Administrator' , 'Administrator');
	
	$client = new NuxeoPhpAutomationClient('http://localhost:8080/nuxeo/site/automation', $interceptor) ;
	$session = $client->getSession ();

	$query = "SELECT * FROM Document WHERE ecm:path STARTSWITH '" . $path . "'";
	
	$answer = $session->newRequest ( "Document.Query" )->set ( 'params', 'query', $query)->setSchema ( '*' )->sendRequest ();
